...
logistic regression and apply it to the task of determining
...
1. (5 pts) Use a schedule to update the learning rate.
    - Supply an appropriate argument to step parameter
    - Support it in your `sg update()` function.
    - Show the effect in your analysis document
2.  (5 pts) Use document frequency (provided in the vocabulary file) to modify the feature values to [tf-idf](https://en.wikipedia.org/wiki/Tf%E2%80%93idf).
...
1. Submit your `analysis.pdf` file
    ...
    - plots are better than text
    - include your name on the PDF
    - clearly enumerate which parts of the assignment you did or did not complete.

Unit Tests
=

I've provided unit tests.  Before running your code on read data, make sure it passes all of the unit tests.  The unit tests run two tests, so if only one fails, one is succeeding.  One tests unregularized logistic regression; the other tests the lazy regularized updates.
...
2.  Certainly make sure that you do the unregularized version first
    and get it to work well.
3.  Use numpy functions whenever you can to make the computation faster.



This assignment and code are derivative of Jordan Boyd-Graber's assignment.
